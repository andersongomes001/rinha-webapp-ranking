  ...
  const [rankingData, setRankingData] = useState<RankingData[]>([])
    ...
    const fetchRankingData = async (showRefreshLoader = false) => {
        ...
        try {
            ...
            const response = await fetch("https://rinha-api-ranking.onrender.com/ranking", {
                cache: "no-store",
            ...
            setError("Não foi possível conectar com a API. Exibindo dados de exemplo.")
        } finally {
            setIsLoading(false)
        ...
        return (
            <div
                ...
                <div className="text-center">
                    ...
                    <p className="text-slate-600 dark:text-slate-400">Carregando ranking...</p>
                </div>
            </div>
        )
    }
            ...
            <div className="container mx-auto px-4 py-8 max-w-7xl">
                <div className="text-center mb-8">
                    <div className="flex items-center justify-center gap-4 mb-4">
                        <h1 className="text-4xl font-bold text-slate-900 dark:text-slate-100">🏆 Ranking da Rinha de
                            Backend 2025</h1>
                    ...
                    <div>
                        ...
                        <Alert variant="destructive" className="text-lg font-semibold max-w-2xl mx-auto mb-3">
                            <AlertTitle>Importante!</AlertTitle>
                            <AlertDescription>
                                A data limite para submeter seu backend é 2025-08-17 até as 23:59:59! A previsão para a divulgação dos resultados é para o dia 2025-08-20.
                            </AlertDescription>
                        </Alert>

                        <Button variant="outline" size={"sm"} asChild>
                            <a href={"https://github.com/zanfranceschi/rinha-de-backend-2025"} target={"_blank"}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <Card>
                        ...
                        <CardContent>
                            <div
                                className="text-2xl font-bold text-slate-900 dark:text-slate-100">{filteredData.length}</div>
                            {filteredData.length !== rankingData.length && (
                                <div className="text-sm text-slate-500">de {rankingData.length} total</div>
                            )}
                        </CardContent>
                    </Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-green-600">
                                ...
                                {filteredData.length > 0
                                    ? Math.max(...filteredData.map((p: RankingData) => p.data.total_liquido)).toLocaleString("pt-BR", {
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-blue-600">
                                {filteredData.length > 0 ? [...new Set(filteredData.flatMap((p: RankingData) => p.langs))].length : 0}
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <RankingFilters data={rankingData} onFilterChange={setFilteredData} isRefreshing={isRefreshing} handleRefresh={handleRefresh}/>
