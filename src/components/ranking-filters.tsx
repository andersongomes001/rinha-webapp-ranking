...
export function RankingFilters({data, onFilterChange, isRefreshing, handleRefresh}: RankingFiltersProps) {
    ...
    const [selectedLoadBalancer, setSelectedLoadBalancer] = useState<string>("todos")
...
// Debounce maxValueInput
    ...
    const filterOptions = useMemo(() => {
        ...
        const messaging = [...new Set(data.flatMap((item) => item.messaging || []))]
            ...
            .sort()
        const loadBalancers = [...new Set(data.flatMap((item) => item["load-balancers"] || []))]
    ...
    useEffect(() => {
        ...
        if (selectedLanguage !== "todos") {
            filtered = filtered.filter((item) => {
                console.log(item.langs)

                return item.langs?.some((lang : string) => lang.toUpperCase() === selectedLanguage.toUpperCase()) ?? false;
            })
        }
        ...
        if (selectedStorage !== "todos") {
            filtered = filtered.filter((item) => {
                return item.storages?.some((storage : string) => storage.toUpperCase() === selectedStorage.toUpperCase()) ?? false;
            })
        }
        ...
        if (selectedMessaging !== "todos") {
            filtered = filtered.filter((item) => {
                return item.messaging?.some((msg : string) => msg.toUpperCase() === selectedMessaging.toUpperCase()) ?? false;
            })
        }
        ...
        if (selectedLoadBalancer !== "todos") {
            filtered = filtered.filter((item) => {
                return item["load-balancers"]?.some((lb : string) => lb.toUpperCase() === selectedLoadBalancer.toUpperCase()) ?? false;
            })
        }
        ...
        <Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    ...
                    <div className={"flex gap-2"}>
                        ...
                        {hasActiveFilters && (
                            <Button variant="outline" size="sm" onClick={clearAllFilters} disabled={isRefreshing} className={"cursor-pointer"}>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    <div>
                        <label
                            className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Linguagem</label>
                        <Select value={selectedLanguage} onValueChange={setSelectedLanguage} disabled={isRefreshing}>
                    ...
                    <div>
                        <label
                            className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Storage</label>
                        <Select value={selectedStorage} onValueChange={setSelectedStorage} disabled={isRefreshing}>
                            <SelectTrigger className="w-full">
                    ...
                    <div>
                        <label
                            className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Messaging</label>
                        <Select value={selectedMessaging} onValueChange={setSelectedMessaging} disabled={isRefreshing}>
                            <SelectTrigger className="w-full">
                                <SelectValue placeholder="Todos"/>
                            </SelectTrigger>
                            <SelectContent>
                    ...
                    <div>
                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Load
                            Balancer</label>
                        <Select value={selectedLoadBalancer} onValueChange={setSelectedLoadBalancer}
                                disabled={isRefreshing}>
                            <SelectTrigger className="w-full">
                                <SelectValue placeholder="Todos"/>
                            </SelectTrigger>
                    ...
                    <div>
                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Valor
                            (R$)</label>
                        <div className="flex gap-2">
                            <Input
                                placeholder="Min"
                ...
                {hasActiveFilters && (
                    <div className="flex flex-wrap gap-2 pt-2 border-t">
                        ...
                        {selectedLanguage !== "todos" && (
                            <Badge variant="secondary" className="flex items-center gap-1">
                                {selectedLanguage}
                                <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedLanguage("todos")}/>
                            </Badge>
                        )}
                        {selectedStorage !== "todos" && (
                            <Badge variant="secondary" className="flex items-center gap-1">
                                Storage: {selectedStorage}
                                <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedStorage("todos")}/>
                            </Badge>
                        )}
                        {selectedMessaging !== "todos" && (
                            <Badge variant="secondary" className="flex items-center gap-1">
                                Messaging: {selectedMessaging}
                                <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedMessaging("todos")}/>
                            </Badge>
                        )}
                        {selectedLoadBalancer !== "todos" && (
                            <Badge variant="secondary" className="flex items-center gap-1">
                                LB: {selectedLoadBalancer}
                                <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedLoadBalancer("todos")}/>
                            </Badge>
                        )}
                        {(minValue || maxValue) && (
                            <Badge variant="secondary" className="flex items-center gap-1">
